...
export default function GalleryPage() {
  ...
  const fetchPhotos = async () => {
    try {
      ...
      const response = await fetch(
        `http://rekogrinikfrontbeck-production-cf17.up.railway.app/api/v1/public/gallery/albums/general`,
        {
          headers: {
  ...
  const deletePhoto = async (photoId: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://rekogrinikfrontbeck-production-cf17.up.railway.app/api/v1/gallery/photos/${photoId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
